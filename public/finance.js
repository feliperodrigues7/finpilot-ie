(function () {
  ...
  const toISO = (ddmmaa) => { if (!ddmmaa) return ''; if (/^\d{4}-\d{2}-\d{2}$/.test(ddmmaa)) return ddmmaa; const m = ddmmaa.match(/^(\d{2})\/(\d{2})\/(\d{4})$/); if (!m) return ddmmaa; return `${m[3]}-${m[2]}-${m[1]}`; };
  const fmtDate = (iso) => { if (!iso) return ''; const d = /^\d{4}-\d{2}-\d{2}$/.test(iso) ? new Date(iso+'T12:00:00') : new Date(iso); if (isNaN(d)) return ''; const dd = String(d.getDate()).padStart(2,'0'); const mm = String(d.getMonth()+1).padStart(2,'0'); const yyyy = d.getFullYear(); return `${dd}/${mm}/${yyyy}`; };
  ...
  function load(){ try{ const raw = localStorage.getItem(STORAGE_KEY); if(!raw){
        state.contas=[{id:uid(),titular:'Giseli',banco:'AIB',tipo:'Corrente',email:'giseli@example.com'},{id:uid(),titular:'FR',banco:'Revolut',tipo:'Poupança',email:'fr@example.com'}];
        ...
        state.fixas=[{id:uid(),nome:'Internet',titular:'FR',valor:40,data:todayISO(),semana:'1',status:'paga'}];
        state.dividas=[];
  ...
  function computeBalances(){ const map=new Map(); const add=(titular,banco,delta)=>{ const key=`${titular}||${banco}`; map.set(key, (map.get(key)||0) + Number(delta||0)); };
    ...
    state.transferencias.forEach(t=>{ const tipo=(t.tipo||'').toLowerCase(); const v=Number(t.valor||0);
  ...
  function renderDashboard(){ const rows=computeBalances(); const tbody=qs('#tblDash tbody'); if(tbody){ tbody.innerHTML=''; rows.forEach(r=>{ const tr=document.createElement('tr'); tr.innerHTML=`<td>${r.titular}</td><td>${r.banco}</td><td><strong>${fmtEUR(r.saldo)}</strong></td>`; tbody.appendChild(tr); }); } const cards=qs('#dashCards'); if(cards){ cards.innerHTML=''; rows.forEach(r=>{ const positive=(r.saldo||0)>=0; const div=document.createElement('div'); div.className='card'; div.innerHTML=`<div class="title">${r.titular} • ${r.banco}</div><div class="value" style="color:${positive?'var(--ok)':'var(--danger)'}">${fmtEUR(r.saldo)}</div><div class="delta muted">${positive?'Saldo positivo':'Saldo negativo'}</div>`; cards.appendChild(div); }); } }

  function initNav(){ const nav = qs('#nav'); const sections = { dashboard: qs('#tab-dashboard'), salarios: qs('#tab-salarios'), fixas: qs('#tab-fixas'), dividas: qs('#tab-dividas'), gastos: qs('#tab-gastos'), transferencias: qs('#tab-transferencias'), categorias: qs('#tab-categorias'), contas: qs('#tab-contas') };
    function activate(tab){ qsa('.nav a').forEach(a=> a.classList.toggle('active', a.dataset.tab===tab)); Object.entries(sections).forEach(([k,el])=> el && el.classList.toggle('hidden', k!==tab)); if(tab==='dashboard') renderDashboard(); if(tab==='contas'){ wireContas(); renderContas(); } if(tab==='gastos') renderGastos(); if(tab==='salarios') renderSalarios(); if(tab==='transferencias') renderTransfers(); if(tab==='categorias') renderCategorias(); if(tab==='fixas') renderFixas(); if(tab==='dividas') renderDividas(); }
    nav.addEventListener('click', (e)=>{ const a=e.target.closest('a[data-tab]'); if(!a) return; e.preventDefault(); activate(a.dataset.tab); history.replaceState(null,'',`#${a.dataset.tab}`); }); const hash=location.hash.replace('#',''); const allowed=['dashboard','salarios','fixas','dividas','gastos','transferencias','categorias','contas']; const initial = allowed.includes(hash)?hash:'dashboard'; activate(initial); }

  const modal = { open(opts){ this.opts = opts; qs('#modalTitle').textContent = opts.title || 'Editar'; const form=qs('#modalForm'); form.innerHTML=''; form.onsubmit = (ev)=>{ ev.preventDefault(); const data=Object.fromEntries(new FormData(form).entries()); if(opts.onSubmit) opts.onSubmit(data); modal.close(); }; form.append(...opts.fields.map(renderField)); const bd=qs('#backdrop'); bd.style.display='flex'; bd.style.alignItems='center'; bd.style.justifyContent='center'; }, close(){ const bd=qs('#backdrop'); bd.style.display='none'; this.opts=null; } };
  ...
  // CONTAS
  function renderContas(){ const tbody=qs('#tblContas tbody'); if(!tbody) return; tbody.innerHTML=''; state.contas.forEach(c=>{ const tr=document.createElement('tr');
  ...
  function wireContas(){
    ...
    const newBtn=qs('#btnNewConta'); const table=qs('#tblContas');
  ...
  // GASTOS
  function renderGastos(){ const tbody=qs('#tblGastos tbody'); if(!tbody) return; tbody.innerHTML='';
    state.gastos.forEach(g=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${fmtDate(g.data||'')}</td><td>${g.titular||''}</td><td>${g.banco||''}</td><td>${g.descricao||''}</td><td>${fmtEUR(g.valor)}</td><td><span class="pill">${g.status||'aberta'}</span></td><td class="actions"><button class="btn ghost" data-act="edit-gasto" data-id="${g.id}">Editar</button><button class="btn danger" data-act="del-gasto" data-id="${g.id}">Excluir</button></td>`;
      tbody.appendChild(tr);
    });
  }
  function wireGastos(){
    qs('#btnNewGasto').addEventListener('click', ()=>{
      ...
      modal.open({
        ...
        onSubmit:(data)=>{
          ...
          state.gastos.unshift({ id:uid(), data:toISO(data.data), titular:data.titular, banco: conta? `${conta.banco}`:'', descricao:data.descricao||'', valor:Number(data.valor||0), conta_id:data.banco_id||null, status:data.status||'aberta' });
    ...
    qs('#tblGastos').addEventListener('click',(e)=>{
      ...
      if(act==='del-gasto'){
        ...
        state.gastos = state.gastos.filter(g=> g.id!==id); persist(); renderGastos();
        ...
        modal.open({
          ...
          onSubmit:(data)=>{
            const conta=contaById(data.banco_id);
            Object.assign(g, { data:toISO(data.data), titular:data.titular, banco: conta? `${conta.banco}`:'', descricao:data.descricao||'', valor:Number(data.valor||0), conta_id:data.banco_id||null, status:data.status||'aberta' });
            persist(); renderGastos();
          }
        });
      }
  ...
  // SALÁRIOS
  function renderSalarios(){ const tbody=qs('#tblSalarios tbody'); if(!tbody) return; tbody.innerHTML='';
    state.salarios.forEach(s=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${fmtDate(s.data)}</td><td>${s.titular}</td><td>${s.banco}</td><td>${s.horas??''}</td><td>${fmtEUR(s.valor)}</td><td class="actions"><button class="btn danger" data-act="del-salario" data-id="${s.id}">Excluir</button></td>`;
      tbody.appendChild(tr);
    });
  }
  function wireSalarios(){
    ...
    qs('#tblSalarios').addEventListener('click',(e)=>{
      ...
      if(btn.dataset.act==='del-salario'){
        ...
        const id=btn.dataset.id; state.salarios = state.salarios.filter(s=> s.id!==id); persist(); renderSalarios();
      }
    });
  }

  // TRANSFERÊNCIAS
  function renderTransfers(){ const tbody=qs('#tblTransfers tbody'); if(!tbody) return; tbody.innerHTML='';
    state.transferencias.forEach(t=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${fmtDate(t.data)}</td><td><span class="pill">${t.tipo}</span></td><td>${t.de_label||''}</td><td>${t.para_label||''}</td><td>${fmtEUR(t.valor)}</td><td>${t.descricao||''}</td><td class="actions"><button class="btn danger" data-act="del-transfer" data-id="${t.id}">Excluir</button></td>`;
      tbody.appendChild(tr);
    });
  }
  function wireTransfers(){
    qs('#btnNewTransfer').addEventListener('click', ()=>{
      ...
      modal.open({
        ...
        onSubmit:(data)=>{
          const cDe=contaById(data.de_id); const cPara=contaById(data.para_id);
    ...
    qs('#tblTransfers').addEventListener('click',(e)=>{
      ...
      if(btn.dataset.act==='del-transfer'){
        ...
        const id=btn.dataset.id; state.transferencias = state.transferencias.filter(t=> t.id!==id); persist(); renderTransfers();
      }
    });
  }

  // CATEGORIAS
  function renderCategorias(){ const tbody=qs('#tblCategorias tbody'); if(!tbody) return; tbody.innerHTML='';
    state.categorias.forEach(c=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${c.nome}</td><td><span class="pill">${c.tipo}</span></td><td class="actions"><button class="btn ghost" data-act="edit-categoria" data-id="${c.id}">Editar</button><button class="btn danger" data-act="del-categoria" data-id="${c.id}">Excluir</button></td>`;
      tbody.appendChild(tr);
    });
  }
    ...
    qs('#tblCategorias').addEventListener('click',(e)=>{
      ...
      if(act==='del-categoria'){
        ...
        modal.open({
          ...
          onSubmit:(data)=>{ c.nome=data.nome; c.tipo=data.tipo; persist(); renderCategorias(); }
        });
      }
    });
  }

  // FIXAS (agora só Semana 1–5)
  const SEMANAS=['1','2','3','4','5'];
  function renderFixas(){ const tbody=qs('#tblFixas tbody'); if(!tbody) return; tbody.innerHTML='';
    state.fixas.forEach(f=>{
      const regra = (f.semana) ? `${f.semana}ª` : '';
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${f.nome}</td><td>${f.titular}</td><td>${fmtEUR(f.valor)}</td><td>${fmtDate(f.data||'')}</td><td>${regra}</td><td><span class="pill">${f.status||'aberta'}</span></td><td class="actions"><button class="btn ghost" data-act="edit-fixa" data-id="${f.id}">Editar</button><button class="btn danger" data-act="del-fixa" data-id="${f.id}">Excluir</button></td>`;
      tbody.appendChild(tr);
    });
  }
  function wireFixas(){
    qs('#btnNewFixa').addEventListener('click', ()=>{
      ...
      modal.open({
        ...
        onSubmit:(data)=>{
          state.fixas.unshift({ id:uid(), nome:data.nome, titular:data.titular, valor:Number(data.valor||0), data: data.data? toISO(data.data):'', semana:data.semana||'', status:data.status||'aberta' });
          persist(); renderFixas();
        }
      });
    });
    qs('#tblFixas').addEventListener('click',(e)=>{
      ...
      if(act==='del-fixa'){
        ...
        state.fixas = state.fixas.filter(f=> f.id!==id); persist(); renderFixas();
        ...
        modal.open({
          ...
          onSubmit:(data)=>{
            Object.assign(f,{ nome:data.nome, titular:data.titular, valor:Number(data.valor||0), data:data.data? toISO(data.data):'', semana:data.semana||'', status:data.status||'aberta' });
            persist(); renderFixas();
          }
        });
      }
  ...
  // DÍVIDAS (semana 1–5 + vencimento opcional)
  function renderDividas(){ const tbody=qs('#tblDividas tbody'); if(!tbody) return; tbody.innerHTML='';
    state.dividas.forEach(d=>{
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${d.nome||''}</td><td>${d.titular||''}</td><td>${fmtEUR(d.valor)}</td><td>${fmtDate(d.vencimento||'')}</td><td><span class="pill">${d.status||''}</span></td><td class="actions"><button class="btn ghost" data-act="edit-divida" data-id="${d.id}">Editar</button><button class="btn danger" data-act="del-divida" data-id="${d.id}">Excluir</button></td>`;
      tbody.appendChild(tr);
    });
  }
  function wireDividas(){
    qs('#btnNewDivida').addEventListener('click', ()=>{
      ...
      modal.open({
        ...
        onSubmit:(data)=>{
          state.dividas.unshift({ id:uid(), nome:data.nome, titular:data.titular||'', valor:Number(data.valor||0), semana:data.semana||'', vencimento:data.vencimento? toISO(data.vencimento):'', status:data.status||'aberta' });
    ...
    qs('#tblDividas').addEventListener('click',(e)=>{
      ...
      if(act==='del-divida'){
        ...
        state.dividas = state.dividas.filter(d=> d.id!==id); persist(); renderDividas();
        ...
        modal.open({
          ...
          onSubmit:(data)=>{
            Object.assign(d,{ nome:data.nome, titular:data.titular||'', valor:Number(data.valor||0), semana:data.semana||'', vencimento:data.vencimento? toISO(data.vencimento):'', status:data.status||'aberta' });
            persist(); renderDividas();
          }
        });
      }
