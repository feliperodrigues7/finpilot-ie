...
function renderPeopleList() {
    ...
    if (people.length === 0) {
        peopleList.innerHTML = '<li class="text-gray-500">Nenhuma pessoa cadastrada.</li>';
        return;
    }
    people.forEach(person => {
        ...
        li.innerHTML = `
            ...
            <button data-name="${person.name}" class="delete-person-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
        `;
...
function renderAccounts() {
    ...
    if (accounts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center py-4 text-gray-500">Nenhuma conta cadastrada.</td></tr>';
        return;
    }
    accounts.forEach(account => {
        ...
        tr.innerHTML = `
            <td class="py-2 px-4">${account.owner_name || 'N/A'}</td>
            <td class="py-2 px-4">${account.name}</td>
            <td class="py-2 px-4">${formatCurrency(account.balance)}</td>
            <td class="py-2 px-4">
                <button data-id="${account.id}" class="edit-account-button text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                <button data-id="${account.id}" class="delete-account-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
            </td>
        `;
    ...
    document.querySelectorAll('.edit-account-button').forEach(button => {
        button.onclick = (e) => editAccount(parseInt(e.target.dataset.id || e.target.closest('button').dataset.id));
    });
    document.querySelectorAll('.delete-account-button').forEach(button => {
        button.onclick = (e) => deleteAccount(parseInt(e.target.dataset.id || e.target.closest('button').dataset.id));
    });
}
...
function renderTransactions() {
    ...
    transactions.forEach(transaction => {
        ...
        tr.className = 'border-b';
        tr.innerHTML = `
            <td class="py-2 px-4">${transaction.owner_name || 'N/A'}</td>
            <td class="py-2 px-4">${formatDate(transaction.date)}</td>
            <td class="py-2 px-4">${transaction.description}</td>
            <td class="py-2 px-4 ${parseFloat(transaction.amount) < 0 ? 'text-red-600' : 'text-green-600'}">${formatCurrency(transaction.amount)}</td>
            <td class="py-2 px-4">${transaction.accounts?.name || 'N/A'}</td>
            <td class="py-2 px-4">${transaction.categories?.name || 'N/A'}</td>
            <td class="py-2 px-4">
                <button data-id="${transaction.id}" class="edit-transaction-button text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                <button data-id="${transaction.id}" class="delete-transaction-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
function renderCategories() {
    ...
    categories.forEach(category => {
        ...
        tr.className = 'border-b';
        tr.innerHTML = `
            <td class="py-2 px-4">${category.owner_name || 'N/A'}</td>
            <td class="py-2 px-4">${category.name}</td>
            <td class="py-2 px-4">${category.type === 'income' ? 'Receita' : 'Despesa'}</td>
            <td class="py-2 px-4">
                <button data-id="${category.id}" class="edit-category-button text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                <button data-id="${category.id}" class="delete-category-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
function renderRecurringRules() {
    ...
    if (recurringRules.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-gray-500">Nenhuma recorrência cadastrada.</td></tr>';
        return;
    }
    recurringRules.forEach(rule => {
        ...
        tr.className = 'border-b';
        tr.innerHTML = `
            <td class="py-2 px-4">${rule.owner_name || 'N/A'}</td>
            <td class="py-2 px-4">${rule.title}</td>
            <td class="py-2 px-4 ${parseFloat(rule.amount) < 0 ? 'text-red-600' : 'text-green-600'}">${formatCurrency(rule.amount)}</td>
            <td class="py-2 px-4">${rule.frequency === 'weekly' ? 'Semanal' : rule.frequency === 'biweekly' ? 'Quinzenal' : 'Mensal'}</td>
            <td class="py-2 px-4">${nextRun ? formatDate(nextRun) : 'N/A'}</td>
            <td class="py-2 px-4">
                <button data-id="${rule.id}" class="edit-recurring-button text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                <button data-id="${rule.id}" class="delete-recurring-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
function renderUpcomingRecurring() {
    ...
    upcoming.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <span>${formatCurrency(item.rule.amount)} em ${formatDate(item.nextRun)}</span>
        `;
    ...
    budgets.forEach(budget => {
        ...
        div.innerHTML = `
            <div class="flex justify-between items-center mb-2">
                <h3 class="font-bold text-lg">${budget.categories.name} (${budget.owner_name || 'N/A'})</h3>
            ...
            <div class="flex justify-end mt-2">
                <button data-id="${budget.id}" class="edit-budget-button text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                <button data-id="${budget.id}" class="delete-budget-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
            </div>
        `;
...
function renderDebts() {
    ...
    tbody.innerHTML = '';
    if (debts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="text-center py-4 text-gray-500">Nenhuma dívida cadastrada.</td></tr>';
        return;
    }
    debts.forEach(debt => {
        const tr = document.createElement('tr');
        tr.className = 'border-b';
        tr.innerHTML = `
            <td class="py-2 px-4">${debt.owner_name || 'N/A'}</td>
            <td class="py-2 px-4">${debt.name}</td>
            <td class="py-2 px-4">${formatCurrency(debt.original_amount)}</td>
            <td class="py-2 px-4">${formatCurrency(debt.current_amount)}</td>
            <td class="py-2 px-4">${(debt.interest_rate * 100).toFixed(2)}% (${debt.interest_type === 'simple' ? 'Simples' : 'Composto'})</td>
            <td class="py-2 px-4">${formatCurrency(calculateEstimatedMonthlyInterest(debt))}</td>
            <td class="py-2 px-4">${debt.due_date ? formatDate(debt.due_date) : 'N/A'}</td>
            <td class="py-2 px-4">${debt.is_closed ? 'Fechada' : 'Aberta'}</td>
            <td class="py-2 px-4">
                <button data-id="${debt.id}" class="edit-debt-button text-blue-500 hover:text-blue-700 mr-2"><i class="fas fa-edit"></i></button>
                <button data-id="${debt.id}" class="delete-debt-button text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
function updateAccountSelectors() {
    ...
    selectors.forEach(selector => {
        ...
        if (currentValue && accounts.some(a => a.id === parseInt(currentValue))) {
            selector.value = currentValue;
        } else if (accounts.length > 0) {
            selector.value = accounts[0].id;
        }
    });
}
...
async function loadTransactionForm(transaction = null) {
    ...
    document.getElementById('transaction-account').value = transaction ? transaction.account_id : (accounts.length > 0 ? accounts[0].id : '');
...
async function saveTransaction(e) {
    ...
    const account_id = parseInt(document.getElementById('transaction-account').value);
    ...
    if (id) {
        ...
        if (error) {
            ...
            const newAccount = accounts.find(a => a.id === account_id);
            if (newAccount) {
...
async function loadRecurringRuleForm(rule = null) {
    ...
    document.getElementById('recurring-account').value = rule ? rule.account_id : (accounts.length > 0 ? accounts[0].id : '');
...
async function saveRecurringRule(e) {
    ...
    const account_id = parseInt(document.getElementById('recurring-account').value);
...
async function exportTransactionsToCsv() {
    ...
    const headers = ['date', 'description', 'amount', 'owner_name', 'account_name', 'category_name'];
    ...
    data.forEach(item => {
        const row = [
            ...
            item.amount,
            `"${String(item.owner_name).replace(/"/g, '""')}"`,
            `"${String(item.accounts?.name || '').replace(/"/g, '""')}"`,
            `"${String(item.categories?.name || '').replace(/"/g, '""')}"`
        ];
    ...
    link.download = `transacoes_finpilot_${new Date().toISOString().slice(0, 10)}.csv`;
...
// --- Reports ---
let cashFlowChart, expensesByCategoryChart, accountBalanceChart;

async function renderReports() {
    const monthStart = getMonthStart(new Date(currentReportMonth));
    const monthEnd = getMonthEnd(new Date(currentReportMonth));

    let transactionsQuery = supa.from('transactions').select('amount, date, categories(name, type), owner_name').eq('user_id', currentUserId);
    if (currentOwnerFilter) {
        transactionsQuery = transactionsQuery.eq('owner_name', currentOwnerFilter);
    }
    transactionsQuery = transactionsQuery.gte('date', monthStart.toISOString().slice(0, 10)).lte('date', monthEnd.toISOString().slice(0, 10));
    const { data: reportTransactions, error: transError } = await transactionsQuery;

    if (transError) {
        console.error('Error fetching report transactions:', transError.message);
        return;
    }
    ...
    const cashFlowLabels = sortedDates;
    const cashFlowIncome = sortedDates.map(date => dailyData[date].income);
    const cashFlowExpenses = sortedDates.map(date => dailyData[date].expense);

    if (cashFlowChart) cashFlowChart.destroy();
    cashFlowChart = new Chart(document.getElementById('cash-flow-chart'), {
        type: 'line',
        data: {
            labels: cashFlowLabels,
            datasets: [
                { label: 'Receitas', data: cashFlowIncome, borderColor: 'rgb(75, 192, 192)', tension: 0.1, fill: false },
                { label: 'Despesas', data: cashFlowExpenses, borderColor: 'rgb(255, 99, 132)', tension: 0.1, fill: false }
            ]
        },
    ...
    reportTransactions.filter(t => t.categories.type === 'expense').forEach(t => {
    ...
    expensesByCategoryChart = new Chart(document.getElementById('expenses-by-category-chart'), {
        type: 'pie',
        data: {
    ...
    let accountBalanceQuery = supa.from('accounts').select('name, balance, owner_name').eq('user_id', currentUserId);
    if (currentOwnerFilter) {
        accountBalanceQuery = accountBalanceQuery.eq('owner_name', currentOwnerFilter);
    }
    const { data: accountBalances, error: accBalanceError } = await accountBalanceQuery;

    if (accBalanceError) {
        console.error('Error fetching account balances:', accBalanceError.message);
        return;
    }

    const accountBalanceLabels = accountBalances.map(a => `${a.name} (${a.owner_name || 'N/A'})`);
    const accountBalanceValues = accountBalances.map(a => parseFloat(a.balance));

    if (accountBalanceChart) accountBalanceChart.destroy();
    accountBalanceChart = new Chart(document.getElementById('account-balance-chart'), {
        type: 'bar',
        data: {
            labels: accountBalanceLabels,
            datasets: [{
                label: 'Saldo',
                data: accountBalanceValues,
                backgroundColor: accountBalanceValues.map(val => val >= 0 ? 'rgb(75, 192, 192)' : 'rgb(255, 99, 132)')
            }]
        },
        options: { responsive: true, scales: { y: { beginAtZero: true } } }
    });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.tab-button').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            if (e.target.dataset.tab === 'reports') {
                renderReports(); // Re-render charts when tab is active
            }
        });
    });

    // Owner Filter
    document.getElementById('owner-filter').addEventListener('change', async (e) => {
    ...
    // Accounts
    document.getElementById('add-account-button').addEventListener('click', () => accountModal.open());

    // Transactions
    document.getElementById('add-transaction-button').addEventListener('click', () => transactionModal.open());
    document.getElementById('transaction-start-date').value = currentTransactionStartDate;
    document.getElementById('transaction-end-date').value = currentTransactionEndDate;
    document.getElementById('transaction-start-date').addEventListener('change', async (e) => {
        currentTransactionStartDate = e.target.value;
        await fetchTransactions();
    });
    document.getElementById('transaction-end-date').addEventListener('change', async (e) => {
        currentTransactionEndDate = e.target.value;
        await fetchTransactions();
    });
    ...
    document.getElementById('export-transactions-button').addEventListener('click', exportTransactionsToCsv);
    document.getElementById('import-transactions-button').addEventListener('click', () => importCsvModal.open());

    // Categories
    document.getElementById('add-category-button').addEventListener('click', () => categoryModal.open());

    // Recurring Rules
    document.getElementById('add-recurring-button').addEventListener('click', () => recurringModal.open());
    document.getElementById('run-recurring-button').addEventListener('click', runRecurringRules);

    // Budgets
    document.getElementById('add-budget-button').addEventListener('click', () => budgetModal.open());
    document.getElementById('budget-month-filter').addEventListener('change', async (e) => {
        currentReportMonth = e.target.value;
        await fetchBudgets();
    });

    // Debts
    document.getElementById('add-debt-button').addEventListener('click', () => debtModal.open());

    // Reports
    document.getElementById('report-month-filter').addEventListener('change', async (e) => {
        currentReportMonth = e.target.value;
        await renderReports();
    });
});
