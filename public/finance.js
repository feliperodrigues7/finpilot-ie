(function () {
  ...
  function load(){
    try{
      ...
      if(!raw){
        state.contas = [
          { id: uid(), titular: "Giseli", banco: "AIB", tipo: "Corrente" },
          { id: uid(), titular: "FR", banco: "Revolut", tipo: "Poupança" },
        ];
        state.categorias = [
        ...
        state.transferencias = [
          { id: uid(), data: "2025-08-10", tipo: "Transferência", de_label: "Giseli - Corrente (AIB)", para_label: "FR - Poupança (Revolut)", valor: 300, descricao: "300 reais para completar aluguel que sera pago na quinta dia 10/10/2025" },
          { id: uid(), data: "2025-08-10", tipo: "Despesa", de_label: "-", para_label: "-", valor: 520, descricao: "Salário Giseli" },
        ];
        state.fixas = [ { id: uid(), nome: "Convelio", titular: "FR", valor: 47.5, data: "2025-10-19", semana: "1", dia: "Qua" } ];
        persist(); return;
      }
  ...
  function initNav(){
    const nav = qs('#nav');
    const sections = {
      dashboard: qs('#tab-dashboard'), contas: qs('#tab-contas'), gastos: qs('#tab-gastos'), salarios: qs('#tab-salarios'), transferencias: qs('#tab-transferencias'), categorias: qs('#tab-categorias'), fixas: qs('#tab-fixas'), dividas: qs('#tab-dividas') };
  ...
  function renderGastos(){ const tbody=qs('#tblGastos tbody'); if(!tbody) return; tbody.innerHTML=''; state.gastos.forEach(g=>{ const tr=document.createElement('tr'); tr.innerHTML=`
        <td>${g.data||''}</td><td>${g.titular||''}</td><td>${g.banco||''}</td><td>${g.descricao||''}</td><td>${fmtEUR(g.valor)}</td>
  ...
  function wireGastos(){ qs('#btnNewGasto').addEventListener('click', ()=>{ const people=pessoasFromContas(); const contas=contasOptions(); modal.open({ title:'Novo gasto', fields:[ { label:'Data', name:'data', type:'date', required:true, value:today() }, { label:'Titular', name:'titular', type:'select', options: people.length?people:['Giseli','FR'] }, { label:'Banco (conta)', name:'banco_id', type:'select', options: contas }, { label:'Descrição', name:'descricao', type:'textarea', rows:3 }, { label:'Valor (EUR)', name:'valor', type:'number', step:'0.01', required:true } ], onSubmit:(data)=>{ const conta = contaById(data.banco_id); state.gastos.unshift({ id:uid(), data:data.data, titular:data.titular, banco: conta? `${conta.banco}`:'', descricao:data.descricao||'', valor:Number(data.valor||0), conta_id:data.banco_id||null }); persist(); renderGastos(); } }); }); qs('#tblGastos').addEventListener('click',(e)=>{ const btn=e.target.closest('button[data-act]'); if(!btn) return; const id=btn.dataset.id; const act=btn.dataset.act; if(act==='del-gasto'){ if(!confirm('Tem certeza que deseja excluir este gasto?')) return; state.gastos = state.gastos.filter(g=> g.id!==id); persist(); renderGastos(); } else if(act==='edit-gasto'){ const g=state.gastos.find(x=> x.id===id); if(!g) return; const people=pessoasFromContas(); const contas=contasOptions(); modal.open({ title:'Editar gasto', fields:[ { label:'Data', name:'data', type:'date', required:true, value:g.data }, { label:'Titular', name:'titular', type:'select', options: people.length?people:['Giseli','FR'], value:g.titular }, { label:'Banco (conta)', name:'banco_id', type:'select', options: contas, value:g.conta_id||'' }, { label:'Descrição', name:'descricao', type:'textarea', rows:3, value:g.descricao }, { label:'Valor (EUR)', name:'valor', type:'number', step:'0.01', required:true, value:g.valor } ], onSubmit:(data)=>{ const conta=contaById(data.banco_id); Object.assign(g, { data:data.data, titular:data.titular, banco: conta? `${conta.banco}`:'', descricao:data.descricao||'', valor:Number(data.valor||0), conta_id:data.banco_id||null }); persist(); renderGastos(); } }); } }); }
