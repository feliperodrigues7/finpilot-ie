(function () {
  ...
  function load(){
    try{
      ...
      if(!raw){
        ...
        state.salarios = [ { id: uid(), data: today(), titular: "Giseli", banco: "AIB", horas: 40, valor: 500 }, { id: uid(), data: today(), titular: "FR", banco: "Revolut", horas: 40, valor: 450 } ];
        state.fixas = [ { id: uid(), nome: "Internet", titular: "FR", valor: 40, data: today(), semana: "1", dia: "Qui", status: "paga" } ];
        state.gastos = [ { id: uid(), data: today(), titular: "FR", banco: "AIB", descricao: "Supermercado", valor: 35.9, status: "paga" } ];
        state.dividas = [ { id: uid(), nome: "Pastel", titular: "FR", valor: 20, vencimento: today(), status: "aberta" } ];
        persist(); return;
      }
  ...
  function computeBalances(){
    ...
    // Débitos pagos: gastos
    state.gastos.filter(g=> (g.status||'')==='paga').forEach(g=> add(g.titular, g.banco, -g.valor));
    // Débitos pagos: despesas fixas (não tem banco, então não subtrai por banco; vamos usar banco do titular principal se existir nas contas; se não, categoria sem banco)
    state.fixas.filter(f=> (f.status||'')==='paga').forEach(f=>{
      // encontrar banco padrão do titular (primeira conta do titular)
      const conta = state.contas.find(c=> c.titular===f.titular) || { banco: '—' };
      add(f.titular, conta.banco, -f.valor);
    });
    // Débitos pagos: dívidas
  ...
  function renderDashboard(){
    ...
    if(cards){ cards.innerHTML=''; rows.forEach(r=>{ const div=document.createElement('div'); div.className='card'; div.innerHTML = `<div class="title">${r.titular} • ${r.banco}</div><div class="value">${fmtEUR(r.saldo)}</div>`; cards.appendChild(div); }); }
  }
